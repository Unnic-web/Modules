...
class CurrencyMod(loader.Module):
    ...
    strings = {
        ...
        "keyerror": "ℹ️ <b>Возможно, валюта отсутствует в "
        "базе данных сайта или вы ввели неправильное название.</b>",
        "currency_not_supported": "ℹ️ <b>Данная валюта не поддерживается. "
        "Пожалуйста, проверьте правильность названия.</b>"
    }
    ...
    async def cvcmd(self, message: Message):
        ...
        api_url = f"https://min-api.cryptocompare.com/data/price?fsym={currency}"
        "&tsyms=USD,RUB,UAH,KZT,EUR,BYN,GBP,CHF,JPY,TON,NOT"
    ...
    async def cvicmd(self, message: Message):
        ...
        instruction = (
            "<b>Список валют</b>\n\n"
            f"{'🇺🇸' if not is_premium else
               '<emoji document_id=5202021044105257611>🇺🇸</emoji>'}
            ' <code>USD</code> <b>(Доллар)</b>\n"
            f"{'🇷🇺' if not is_premium else 
               '<emoji document_id=5449408995691341691>🇷🇺</emoji>'}
            ' <code>RUB</code> <b>(Рубль)</b>\n"
            f"{'🇺🇦' if not is_premium else 
               '<emoji document_id=5447309366568953338>🇺🇦</emoji>'} 
            ' <code>UAH</code> <b>(Гривна)</b>\n"
            f"{'🇰🇿' if not is_premium else 
               '<emoji document_id=5228718354658769982>🇰🇿</emoji>'} 
            ' <code>KZT</code> <b>(Тенге)</b>\n"
            f"{'🇪🇺' if not is_premium else 
               '<emoji document_id=5228784522924930237>🇪🇺</emoji>'} 
            ' <code>EUR</code> <b>(Евро)</b>\n"
            f"{'🇧🇾' if not is_premium else 
               '<emoji document_id=5382219601054544127>🇧🇾</emoji>'}
            ' <code>BYN</code> <b>(Бун)</b>\n"
            f"{'🇬🇧' if not is_premium else 
               '<emoji document_id=5202196682497859879>🇬🇧</emoji>'}
            ' <code>GBP</code> <b>(Фунт)</b>\n"
            f"{'🇨🇭' if not is_premium else 
               '<emoji document_id=5442703336266543270>🇨🇭</emoji>'}
            ' <code>CHF</code> <b>(Франк)</b>\n"
            f"{'🇯🇵' if not is_premium else 
               '<emoji document_id=5456261908069885892>🇯🇵</emoji>'}
            ' <code>JPY</code> <b>(Йена)</b>\n"
            f"{'🔵' if not is_premium else 
               '<emoji document_id=5253691721174234015>💎</emoji>'}
            ' <code>TON</code> <b>(Тонкоин)</b>\n"
            f"{'⚫️' if not is_premium else 
               '<emoji document_id=5379965911455256722>💎</emoji>'}
            ' code>NOT</code> <b>(Ноткоин)</b>\n\n"
            "<b>Использовать через:</b> <code>.cv</code>"
            "<число> <название>"
        )
